	...
	public function lessc( $files )
	{
		...
		foreach( $files as $file )
		{
			...
			if( !file_exists( $outputfile ) )
			{
				...
				{
					$importDirs = str_replace( array( ', ', ',' ), array( ':', ':' ), $this->configuration['other']['importDirs'] );
				}

				exec( sprintf( 'lessc %s --line-numbers=\'comments\' --include-path=%s %s > %s', $compressed, $importDirs, $file, $outputfile ) );
